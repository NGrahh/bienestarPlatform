                                                            ...
                                                            <div class="modal-body">
                                                                <div class="card-body">
                                                                    <form action="{{ route('users.destroy', ['id' => $user->id]) }}" method="POST">
                                                                        @csrf
                                                                        @method('DELETE')
                                                                        <p>El usuario <strong>{{ $user->name }} {{ $user->lastname }}</strong>, identificado con el documento de tipo <strong>{{ $user->TypeDocument->name }}</strong> y número <strong>{{ $user->document }}</strong>, será eliminado. Si necesitas recuperar información o tomar alguna medida antes de proceder con la eliminación, ten en cuenta que su correo electrónico es <strong>{{ $user->email }}</strong> y desempeña el rol de <strong>{{ $user->role->name }}</strong> en nuestra organización.</p>
                                                                        <div class="modal-footer d-flex justify-content-center gap-2">
